...
Section~\ref{sec:identification} delivered two risk premiums with inverted compositions, $\Pi_{\text{risk}}^{\text{USDC}} = 48.9$ and $\Pi_{\text{risk}}^{\text{USDT}} = 71.2$ basis points (Table~\ref{tab:dual_asset_premium}). We carry each into the corridor-level cost comparison, applying the USDC premium where USDC is the natural choice and the USDT premium where USDT dominates the real-world flow. The three use cases, set out in Table~\ref{tab:use_cases}, span the policy-relevant space: one corridor where a scaled public rail already exists and two where none does yet.

\begin{table}[H]
\centering
\caption{The three use cases of the empirical comparison.}\label{tab:use_cases}
...
\paragraph{The stablecoin path.} The sender pays the on-chain transfer fee (about 37 basis points for USDT-on-Tron at USD 400, 15 for a low-fee L1/L2 USDC transfer), the destination off-ramp basis, and the structural risk premium of Section~\ref{sec:risk_factors}. One explicit cost is left out of the tables and bounded here instead, for symmetry with the all-in figures of the non-stablecoin rails: the platform fee of the on-ramp and off-ramp themselves. A retail user who enters and exits through an exchange or fiat gateway pays a posted fee of roughly 10 to 50 basis points per leg at retail size (Appendix~\ref{app:cost_data}), on top of the execution costs already priced in the comparison, namely the below-par conversion loss inside $\Pi_{\text{risk}}$ and the local-currency basis $b_{\text{dest}}$. Adding 20 to 100 basis points of platform fees to the stablecoin column tightens the cross-border results without overturning most of them: the US-Mexico margin over Wise narrows from 130 basis points to between 30 and 110, the EU-Brazil USDC margin over Wise from 120 to between 20 and 100, and at the upper end of the range the USDT path on EU-Brazil loses its 98-basis-point edge over Wise, while the domestic dominance of PIX is reinforced. The stablecoin's advantage where it wins is real but thinner than the headline tables suggest for a retail user without direct exchange access.

The total cost for a non-stablecoin rail is
\begin{equation}\label{eq:total_cost_nonstable}
c = \phi + \kappa_W \cdot \tau,
\end{equation}
where $\phi$ is the per-transaction fee in basis points (inclusive of the FX margin published by the operator) and $\kappa_W \cdot \tau$ is the working-capital cost of the settlement delay. For a stablecoin rail the cost adds two empirically observable components: the stablecoin off-ramp basis in the destination jurisdiction $b_{\text{dest}}$ and the structural risk premium $\Pi_{\text{risk}}^{a}$ delivered by the global game,
\begin{equation}\label{eq:total_cost_stable}
c^{\text{stable}} = \phi + b_{\text{dest}} + \Pi_{\text{risk}}^{a}.
\end{equation}
...
Because $b_{\text{dest}}$ and the baseline depeg component of $\Pi_{\text{risk}}^{a}$ both touch the act of conversion, the two could in principle double-count the same cost, so we state explicitly why they do not. The destination basis is a level: the average premium of the stablecoin's local-currency quote over the official USD-quoted rate, a cost the converting sender pays with certainty and that is driven by local liquidity and the compliance cost that market makers pass through. The baseline depeg component is an expected loss: the one-sided, probability-weighted cost of converting the par claim below par on the USD leg, which is zero in most hours and concentrated in hours of elevated redemption pressure. The mechanical overlap between the two is the below-par portion of the USD-pair level already embedded in local-currency quotes, and that portion is bounded by the data: the USD-pair mean deviation from par in normal conditions is below one basis point, and its one-sided expectation is between $0.4$ and $2$ basis points across the two coins. Netting that bound out of the stablecoin totals would lower them by at most two basis points and changes no ranking in any use case, so we report the two terms unnetted.
...
\begin{tabular}{lrrrr}
\toprule
Rail & Fee (bps) & Basis / WC (bps) & Risk (bps) & Total (bps) \\
\midrule
PIX                  & 0.0    & 0.00  & 0.0  & 0.0   \\
TED bank transfer    & 200.0  & 0.62  & 0.0  & 200.6 \\
USDT on Tron         & 375.0  & 21.0  & 71.2 & 467.2 \\
\bottomrule
\end{tabular}
\end{table}

PIX dominates at zero cost. USDT is the most expensive rail at 467 basis points; the flat-fee component at the small transaction value dominates both the off-ramp basis and the structural risk loading. The stablecoin is dominated by an upgraded public rail that already exists.
...
\begin{table}[H]
\centering
\caption{Use case 2: US-Mexico remittance, USDT-on-Tron, transaction value USD 400. SWIFT correspondent and Wise figures are RPW Q3 2025 means; USDT total adds the Mexican peso off-ramp basis.}\label{tab:uc2}
\resizebox{\textwidth}{!}{\begin{tabular}{lrrrr}
\toprule
Rail & Fee (bps) & Basis / WC (bps) & Risk (bps) & Total (bps) \\
\midrule
USDT on Tron                     & 37.5  & 12.0 & 71.2 & 120.7 \\
Wise (fintech remittance)        & 250.0 & 1.23 & 0.0  & 251.2 \\
SWIFT correspondent (Bank avg)   & 331.0 & 2.47 & 0.0  & 333.5 \\
\bottomrule
\end{tabular}}
\end{table}

USDT wins at 121 basis points, beating Wise by 130 basis points and the SWIFT correspondent average by 213 basis points. The stablecoin's advantage is robust to the addition of the empirically measured Mexican off-ramp basis. The fintech-upgraded incumbent (Wise) is closer to the stablecoin than the traditional incumbent (SWIFT): the cross-border payments innovation gap has been partly closed by fintech entry, and the residual gap is what the stablecoin path captures.
...
\begin{table}[H]
\centering
\caption{Use case 3: EU-Brazil cross-border, four rails (SWIFT correspondent, Wise, USDC, USDT), transaction value USD 1,000.}\label{tab:uc3}
\begin{tabular}{lrrrr}
\toprule
Rail & Fee (bps) & Basis / WC (bps) & Risk (bps) & Total (bps) \\
\midrule
USDC cross-border               & 15.0  & 21.0 & 48.9 & 84.9  \\
USDT cross-border               & 15.0  & 21.0 & 71.2 & 107.2 \\
Wise (RPW Q3 2025 mean)         & 204.0 & 1.23 & 0.0  & 205.2 \\
SWIFT correspondent             & 487.0 & 2.47 & 0.0  & 489.5 \\
\bottomrule
\end{tabular}
\end{table}
...
The stablecoin path wins on this corridor: USDC at 85 basis points dominates Wise by 120 and SWIFT by 405; USDT at 107 dominates Wise by 98 and SWIFT by 382. The empirical Brazilian off-ramp basis is small enough that it does not change the ordering, and the regulatory package introduced in November 2025 is not associated with any additional friction in the post-implementation window relative to the secular trend.
...
Where an upgraded public instant-payment rail already serves the corridor, the stablecoin is dominated: on the Brazilian domestic corridor PIX clears at zero cost against the stablecoin's 467 basis points.
\end{result}

\begin{result}[Wins where none exists]\label{res:wins}
Where no upgraded public alternative exists, the stablecoin wins net of its risk premium: 121 basis points against 251 (Wise) and 334 (SWIFT) on US-Mexico, and 85 against 205 and 490 on EU-Brazil, with the fintech-upgraded incumbent the closest competitor.
\end{result}

\begin{result}[Absorbed once an upgraded public cross-border rail operates]\label{res:absorbed}
Once an upgraded public cross-border rail is operational it dominates the stablecoin by one to two orders of magnitude; Section~\ref{sec:discussion} shows a projected BIS Nexus rail at 5 and 2 basis points against the stablecoin's 121 and 85.
\end{result}

The three results trace the same ordering across the policy-relevant space, and they hold for both stablecoins: the per-asset distinction changes the magnitude of the premium, not the regime. The consolidated summary is reported in the internet appendix; the supervisory implication of the inverted USDC and USDT compositions is taken up in Section~\ref{sub:policy_implications}.
