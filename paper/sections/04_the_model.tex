...
The proposition gives a single equilibrium threshold $s^*$ at any admissible parameter vector, so the conversion mass $D^*(\theta)$ is determined at each fundamental and the calibration recovers one risk premium per issuer. The contraction condition is verified numerically at the calibrated parameters.
...
A deeper liquid reserve sustains a higher secondary price at any conversion volume, which raises the value of converting and so weakly raises the equilibrium conversion mass. This is the reserve-quality channel of \citet{ma_zeng_zhang_2025}.
...
Together the two comparative statics formalise the claim of Section~\ref{sub:not_homogeneous} that reserve composition and payment use are primitives that move the priced premium, not stylistic differences between issuers.

\subsection{The risk premium}\label{sub:what_it_delivers}

The output of the model, for each issuer under its own calibration, is three expected per-unit losses in basis points,
\begin{align}
\ell_C &= h \cdot \xi \cdot 10^4,\label{eq:ellC} \\
...
\end{align}
where $\mathbb{E}_\rho$ is the expectation under regime $\rho$. The counterparty component $\ell_C$ is the reduced-form credit term of Section~\ref{sub:counterparty}, independent of the coordination stage; the two depeg components are the structural output of the global game. Their probability-weighted sum is the risk premium the holder bears,
\begin{equation}\label{eq:total_risk_premium}
\Pi_{\text{risk}} = \ell_C + (1-p_s)\,\ell_{D,b} + p_s\,\ell_{D,s},
\end{equation}
which we add to the on-chain transfer fee. Section~\ref{sec:calibration} calibrates this premium separately for the two stablecoins.
