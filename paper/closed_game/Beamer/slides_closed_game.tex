...
Convert the coin back to local fiat: the conversion basis and platform fee are
\emph{circuit fees}, not holding risk. The holding loss is counterparty $+$ stress.
\vspace{0.4em}
\begin{center}
\begin{tabular}{lccc}
\toprule
Holding-loss component & USDC & USDT & source \\
\midrule
\(\ell_C\) counterparty            & 20.0 & 60.0 & market proxies \\
\(\ell_{D,s}\) stress depeg (wtd)   & \phantom{0}1.3 & \phantom{0}0.0 & global game \(\times\,p_s\) \\
\midrule
\(\El\) holding loss               & \textbf{21.3} & \textbf{60.0} & \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.4em}
Both counterparty-led, but separated by \textbf{stress-depeg severity}: USDC's
primary redemption is suspension-prone (deep trough), USDT's stayed open (mild).
\end{frame}
...
\midrule
BR domestic \$40   & USDT & 506 & PIX (0)    & \textcolor{trad}{dominated} \\
US--MX \$400       & USDT & 160 & Wise (251) & \textcolor{stab}{wins by 92} \\
EU--BR \$1{,}000   & USDC & 107 & Wise (205) & \textcolor{stab}{wins by 98} \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.5em}
...
\textbf{Heterogeneity}
\begin{itemize}
  \item USDC: deep stress depeg \(\Rightarrow\) supervise secondary-market depth and primary-redemption access.
  \item USDT: counterparty-led \(\Rightarrow\) supervise reserve composition and attestation.
...
\begin{enumerate}
  ...
  \item We price it leg by leg: counterparty and the conversion basis are \textbf{observable};
        the stress run is the \textbf{global game}, closed form, with an endogenous per-coin \(p_s\).
  \item Holding loss USDC 21 bps vs USDT 60 bps, both counterparty-led but split by stress-depeg severity: \textbf{two instruments, not one category}.
