...
This appendix collects the mechanics of the calibration summarised in Section~\ref{sec:calibration} and the identification diagnostics behind the expected-loss decomposition reported in Section~\ref{sec:identification}. The free parameter vector, the loss function, and the optimisation are documented first; the Jacobian-based identification check, the effective-rank analysis, and the two robustness panels that support the headline follow.
...
\end{equation}
where $\sigma$ is the signal noise, $(\psi, \nu, \mu_q, \zeta)$ is the secondary market block, and $(\mu_s, \tau_s)$ are the stress-regime prior mean and precision. Two parameters are fixed externally on economic grounds: $\mu_b = 1$ (par by definition of the baseline regime) and $\tau_b = 400$, which implies a baseline fundamental standard deviation of $0.05$, roughly two orders of magnitude above the empirical baseline price standard deviation of $0.0008$ but well below the calibrated signal noise $\sigma = 0.194$, so the holder posterior is dominated by $\sigma$ rather than by the prior dispersion. The headline is insensitive to $\tau_b$ within a wide range around this value (internet appendix).
...
\end{equation}
where $m_j(\phi)$ is the model moment under $\phi$ from a Monte Carlo simulation, $\hat m_j$ is the data target, and $w_j$ is the weight. Weights are chosen so that each moment contributes a comparable amount to the unweighted loss at the calibration anchor. A weighting proportional to the inverse sampling variance of the empirical moments would not serve the object here: the baseline regime is quasi-deterministic by construction, so its moments carry near-zero sampling variance and would absorb effectively all the weight, concentrating the fit on the calm regime at the expense of the stress regime the model is built to price; the scale normalisation keeps both regimes informative. The Monte Carlo uses $N=300$ scenario draws for the main pass and a Nelder-Mead simplex search with up to 80 iterations on a warm-start parameter vector obtained from a $N=100$ warm-up pass.
...
\end{pmatrix}.
\end{equation}
The implied moments at $\phi^*$ are $m_1 = 0.995$, $m_2 = 3.8 \times 10^{-4}$, $m_3 = 0.000$, $m_4 = 0.975$, $m_5 = 238.7$ bps, $m_6 = 0.358$. Against the targets of Table~\ref{tab:moments_targets}, two moments are matched within reading tolerance ($m_3$ and $m_5$), two carry quantified residuals ($m_1$, 50 basis points below the par target; $m_4$, $0.975$ against the $0.902$ trough), and two are not matched ($m_2$, at half the empirical baseline dispersion, and $m_6$, $0.358$ against $0.396$). The moment-by-moment interpretation of the fit is in Section~\ref{sub:interp} of the main text, with the residual diagnostics in the internet appendix; the level of the baseline depeg component, the component most exposed to the $m_1$ residual, is anchored against observable conversion costs in Section~\ref{sec:identification}.
...
\end{equation}
The parameter directions match the structural priors in Section~\ref{sub:not_homogeneous} of the main text: USDT exhibits lower price-impact scale $\psi$ and depth-erosion $\zeta$ (deeper, more resilient secondary market), convexity $\nu$ at the upper bound of its admissible range (flat dynamics until depth saturates), and higher signal noise $\sigma$ (broad retail user base across emerging markets). The milder May 2022 episode is absorbed mainly through the low price-impact scale rather than through the stress-regime mean, which is close to the USDC value; the secondary market clears the conversions the Terra fallout generated with far smaller realised discounts. These directions generate the inverted composition of the USDT expected loss relative to USDC reported in Section~\ref{sec:identification}: USDT's small depeg loss leaves its heavier counterparty profile dominant. The stress trough reads as for USDC: the model's deepest stress price for USDT ($0.997$) sits above the empirical minimum close of $0.9751$, a residual the two-class extension in the internet appendix attributes to the secondary market intermediation sector rather than to the payment holder.
...
\begin{table}[H]
\centering
\caption{Per-parameter identification strength at $\phi^*$. Strength is the L2 norm of the parameter's column of the normalised moment Jacobian (Section~\ref{sub:jacobian}); parameters with strength above one are classified as well identified.}\label{tab:identification}
\begin{tabular}{lrl}
\toprule
...
\subsection{Bootstrap standard errors of the two expected losses}\label{sub:bootstrap}

Because each expected loss is calibrated to a single stress window (the 96-hour March 2023 episode for USDC and the 97-hour May 2022 episode for USDT), we quantify the sampling uncertainty of each by a circular block bootstrap of its window. In each of 100 replications we resample the hourly secondary price in twelve-hour blocks, recompute the three stress moments, re-run the calibration warm-started at $\phi^*$, and recompute the expected loss; the baseline moments and the Monte Carlo seeds are held fixed so the spread reflects data resampling rather than simulation noise. The USDC replications are centred on the point estimate (mean $48.7$ against a point value of $48.9$) with a standard deviation of $2.3$ basis points and a $90\%$ percentile interval of $[44.7, 51.7]$; the USDT replications give a mean of $70.4$ against a point value of $71.2$, a standard deviation of $2.5$ basis points, and a $90\%$ interval of $[65.5, 72.8]$. Each expected loss is thus pinned to within about two and a half basis points of its point value by its stress-window data. The bootstrap resamples the depeg window with the counterparty parameters $(h,\xi)$ held fixed; the wider range that varying those externally set parameters implies is reported separately in the internet appendix.
