...
\centering
\begin{tikzpicture}[
  font=\footnotesize, >=Latex, node distance=5mm,
  ...
  hold/.style={box, fill=green!10, minimum height=12mm},
  lab/.style={font=\scriptsize, align=center},
  fee/.style={font=\scriptsize\itshape, align=center}
]
% --- Panel A: traditional rail
\node[lab, anchor=west] at (0,2.55) {\textbf{(a) Traditional rail (SWIFT / RTGS)}};
\node[trad] (ta) at (1.2,1.7) {sender\\deposit};
\node[trad] (tb) at (4.4,1.7) {rail\\(SWIFT/RTGS)};
\node[trad] (tc) at (7.6,1.7) {recipient\\deposit};
\draw[->] (ta) -- node[fee, above]{fee + FX} (tb);
\draw[->] (tb) -- node[fee, above]{settlement} (tc);
\node[lab, anchor=west, text=blue!50!black] at (9.0,1.7) {underlying $=$ deposit\\(par, insured, resolved)\\[2pt]\textbf{expected loss $=0$}};
% --- Panel B: stablecoin circuit
\node[lab, anchor=west] at (0,0.7) {\textbf{(b) Stablecoin circuit (fiat to fiat)}};
\node[stab] (sa) at (0.9,-0.4) {fiat};
\node[stab] (sb) at (3.0,-0.4) {on-ramp};
\node[hold] (sc) at (5.5,-0.4) {hold coin};
\node[stab] (sd) at (8.0,-0.4) {off-ramp};
\node[stab] (se) at (10.2,-0.4) {local\\fiat};
\draw[->] (sa) -- (sb);
\draw[->] (sb) -- node[fee, above]{on-chain} (sc);
\draw[->] (sc) -- (sd);
\draw[->] (sd) -- (se);
\node[fee, below=1mm of sb, text=red!55!black] {fee\\(observable)};
\node[fee, below=1mm of sd, text=red!55!black] {basis + fee\\(observable)};
\node[lab, above=0.5mm of sc, text=green!40!black] {$\ell_C$ counterparty (obs.)\\$+$ stress depeg};
\node[lab, anchor=west, text=green!40!black] at (11.0,-0.4) {stress $=$\\global game};
\end{tikzpicture}
\caption{Two payment workflows. On the traditional rail (a) the payer holds a par-redeemable, insured deposit, so the expected loss on the underlying is zero. On the stablecoin circuit (b) the payer runs fiat to fiat: the on-ramp, on-chain, and off-ramp legs are observable fees (the off-ramp also carries the conversion basis), while the holding leg carries the expected loss, the observable counterparty exposure $\ell_C$ plus the stress depeg, the one piece priced by the closed-form global game.}\label{fig:workflows}
\end{figure}
