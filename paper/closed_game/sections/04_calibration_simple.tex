...
Because the counterparty and baseline pieces are observable, a natural question is what the coordination model adds beyond a purely reduced-form account. We answer it directly. A reduced-form expected loss built only from observables, the counterparty term $h\xi$, the off-ramp basis for the baseline, and the empirical stress frequency times the observed episode depth for the stress piece, gives 43.6 basis points for USDC and 72.3 for USDT. The closed-form model gives 41.0 and 72.0. The two agree, which is the point: the model does not inflate the number, and the headline is disciplined by observables. What the model adds is not the level but the mechanism, the endogenous per-coin stress probability of Proposition~\ref{prop:ps}, the reserve comparative static of Proposition~\ref{prop:reserves}, and the threshold that the reduced-form account cannot deliver and that the policy questions of Section~\ref{sec:discussion} require. The expected depeg loss the model carries in expectation is small, because the deep trough is a tail event; the headline is dominated by the observable counterparty and basis terms, and the model earns its place by pricing how those terms move with reserves, depth, and the regulation that governs them.
