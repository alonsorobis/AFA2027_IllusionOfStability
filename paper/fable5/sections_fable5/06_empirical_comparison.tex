...
Section~\ref{sec:identification} delivered two expected losses with inverted compositions, $\Pi_{\text{risk}}^{\text{USDC}} = 48.9$ and $\Pi_{\text{risk}}^{\text{USDT}} = 71.2$ basis points (Table~\ref{tab:dual_asset_premium}). We carry each into the corridor-level cost comparison, applying the USDC expected loss where USDC is the natural choice and the USDT expected loss where USDT dominates the real-world flow. The three use cases, set out in Table~\ref{tab:use_cases}, span the policy-relevant space: one corridor where a scaled public rail already exists and two where none does yet.
...
\paragraph{The stablecoin path.} The stablecoin path prices the full payment circuit. The sender converts fiat into the stablecoin at an on-ramp, transfers it on chain, and the recipient converts it back into local fiat at an off-ramp, so the cost is the sum of the on- and off-ramp platform fees, the on-chain transfer fee (about 37 basis points for USDT-on-Tron at USD 400, 15 for a low-fee L1/L2 USDC transfer), the destination basis, and the expected loss of Section~\ref{sec:risk_factors}. We set the on- and off-ramp platform fees at 25 basis points each, the central value of the 10-to-50-basis-point per-leg retail range documented in Appendix~\ref{app:cost_data}, and report the round-trip in a dedicated column of the use-case tables. This 50-basis-point round-trip is what a retail user without direct exchange access pays to enter and exit the stablecoin; it is distinct from the two execution costs the comparison already prices, the below-par conversion loss inside $\Pi_{\text{risk}}$ and the local-currency basis $b_{\text{dest}}$. At the upper end of the per-leg range the round-trip doubles to 100 basis points, which narrows the cross-border margins without reversing them, except that the USDT path on EU-Brazil draws level with Wise; the domestic dominance of PIX is only reinforced.
...
USDT on Tron         & 375.0  & 50.0 & 21.0  & 71.2 & 517.2 \\
\bottomrule
\end{tabular}
\end{table}

PIX dominates at zero cost. USDT is the most expensive rail at 517 basis points; the flat-fee component at the small transaction value dominates the round-trip ramp fees, the off-ramp basis, and the expected-loss loading alike. The stablecoin is dominated by an upgraded public rail that already exists.
...
Rail & Fee (bps) & Ramp (bps) & Basis / WC (bps) & Risk (bps) & Total (bps) \\
\midrule
USDT on Tron                     & 37.5  & 50.0 & 12.0 & 71.2 & 170.7 \\
...
USDT wins at 171 basis points, beating Wise by 80 basis points and the SWIFT correspondent average by 163 basis points. The advantage survives the full payment circuit, the round-trip ramp fees and the Mexican off-ramp basis included. The fintech-upgraded incumbent (Wise) is closer to the stablecoin than the traditional incumbent (SWIFT): the cross-border payments innovation gap has been partly closed by fintech entry, and the residual gap is what the stablecoin path captures.
...
Rail & Fee (bps) & Ramp (bps) & Basis / WC (bps) & Risk (bps) & Total (bps) \\
\midrule
USDC cross-border               & 15.0  & 50.0 & 21.0 & 48.9 & 134.9 \\
USDT cross-border               & 15.0  & 50.0 & 21.0 & 71.2 & 157.2 \\
...
The stablecoin path wins on this corridor: USDC at 135 basis points dominates Wise by 70 and SWIFT by 355; USDT at 157 dominates Wise by 48 and SWIFT by 332. The full payment circuit, ramp fees and off-ramp basis included, leaves the ordering intact, and the regulatory package introduced in November 2025 is not associated with any additional friction in the post-implementation window relative to the secular trend.
...
Where an upgraded public instant-payment rail already serves the corridor, the stablecoin is dominated: on the Brazilian domestic corridor PIX clears at zero cost against the stablecoin's 517 basis points.
\end{result}

\begin{result}[Wins where none exists]\label{res:wins}
Where no upgraded public alternative exists, the stablecoin wins net of its expected loss and the full conversion circuit: 171 basis points against 251 (Wise) and 334 (SWIFT) on US-Mexico, and 135 against 205 and 490 on EU-Brazil, with the fintech-upgraded incumbent the closest competitor.
\end{result}

\begin{result}[Absorbed once an upgraded public cross-border rail operates]\label{res:absorbed}
Once an upgraded public cross-border rail is operational it dominates the stablecoin by one to two orders of magnitude; Section~\ref{sec:discussion} shows a projected BIS Nexus rail at 5 and 2 basis points against the stablecoin's 171 and 135.
\end{result}
